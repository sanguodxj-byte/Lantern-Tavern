...
var target_room_count: int = 18  # isaac 目标房间数，clamp 到 [6,18]
...
var ceiling_transition_gap: float = 0.015   # 兼容保留的过渡容差；正式地形不得用它制造实际空隙
...
var ceiling_height_base: float = MIN_CEILING_HEIGHT_METERS  # 地形默认天花板高度（米）
...
## 将任意旧高度映射到整数米层；低于可行走空间最低高度时取最低层。
static func quantize_height(raw_height: float) -> float:
	var layer := roundi(raw_height / HEIGHT_UNIT_METERS)
	return maxf(float(layer) * HEIGHT_UNIT_METERS, MIN_CEILING_HEIGHT_METERS)


static func is_integer_height(height: float) -> bool:
	return is_equal_approx(height / HEIGHT_UNIT_METERS, round(height / HEIGHT_UNIT_METERS))


static func normalize_height_grid(raw_grid: Array) -> Array:
	var normalized: Array = []
	for raw_row in raw_grid:
		var row: Array = []
		for raw_height in raw_row:
			row.append(quantize_height(float(raw_height)))
...
func validate() -> Dictionary:
	...
	if not is_integer_height(tile_size):
		errors.append("tile_size must be an integer meter grid unit (%f)" % tile_size)
	...
	for door_name in ["standard_door_size_meters", "boss_door_size_meters"]:
		var door_size: Vector2 = get(door_name)
		if not is_integer_height(door_size.x) or not is_integer_height(door_size.y):
			errors.append("%s must use integer meter dimensions" % door_name)
		if door_size.x >= tile_size:
			errors.append("%s width must fit inside one tile (%f >= %f)" % [door_name, door_size.x, tile_size])
	...
	if not is_integer_height(ceiling_height_base):
		errors.append("ceiling_height_base must use an integer meter layer")
