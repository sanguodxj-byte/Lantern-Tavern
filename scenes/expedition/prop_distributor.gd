...
class RoomNode:
 ...
 var neighbors: Array = []
 var room_tag: String = ""   # 房间标签（可选，用于 tag 关联放置策略）
...
func generate_poisson_distribution(room_size: Vector2, min_dist: float, sample_count: int = 30) -> Array:
 var points = []
 var grid = []
...
# Enhanced: accepts tag_filter to restrict which tag to place in which room type.
func solve_prop_distribution(rooms: Array, start_room: RoomNode, tag_filter: Dictionary = {}) -> Dictionary:
 var results = {
  "portal_room": null,
  "chest_rooms": [],
  "tag_placements": {},  # tag → [RoomNode, ...]
 }
 ...
 if not tag_filter.is_empty():
  results["tag_placements"] = _apply_tag_filter(rooms, tag_filter)

 return results

## 根据 tag_filter 将房间按标签分组。
## tag_filter 格式：{tag_string: {preference: String, ...}}
## 如：{"treasure": {"preference": "dead_end"}, "material": {"preference": "random"}}
func _apply_tag_filter(rooms: Array, tag_filter: Dictionary) -> Dictionary:
 ...
 for tag in tag_filter:
  var filter_cfg: Dictionary = tag_filter[tag]
  var preference: String = filter_cfg.get("preference", "random")
  var target_pool: Array
  match preference:
   "dead_end":
    target_pool = dead_ends.duplicate() if not dead_ends.is_empty() else others.duplicate()
   "far":
    # 取距离远的房间（按 distance_from_start 排序）
    rooms.sort_custom(func(a, b): return a.distance_from_start > b.distance_from_start)
    target_pool = rooms.duplicate()
   _:
    target_pool = rooms.duplicate()
  # 随机选取至多 max_count 个房间
  var max_count: int = filter_cfg.get("max_count", 1)
  var selected: Array = []
  target_pool.shuffle()
  for i in range(min(max_count, target_pool.size())):
   selected.append(target_pool[i])
  placements[tag] = selected
...
func generate_tavern_layout(floor_size: Vector2, table_spacing: float = 3.0) -> Array:
 ...
 while x <= end_x:
  ...
  while y <= end_y:
   ...
   furniture_placements.append({"type": "table", "position": table_pos, "tag": ItemTags.FURNITURE})
   ...
   for offset in stool_offsets:
    furniture_placements.append({"type": "stool", "position": table_pos + offset, "tag": ItemTags.FURNITURE})
