...
func test_capture_crossbow_real_3d_views_in_godot() -> void:
	assert_bool(SUPPORT.real_renderer_available()) \
		.override_failure_message("crossbow material capture requires a non-headless renderer").is_true()
	var packed := load(GLB_PATH) as PackedScene
	assert_object(packed).is_not_null()
	var output_paths := {}
	for view_name in ["preview", "front", "side", "top"]:
		output_paths[view_name] = "%s/voxel_crossbow_godot_%s.png" % [OUTPUT_DIR, view_name]
	...
	for view_name in ["preview", "front", "side", "top"]:
		...
		assert_bool(inspection["nonblank"]) \
			.override_failure_message("blank Godot crossbow capture: %s" % inspection["path"]).is_true()
