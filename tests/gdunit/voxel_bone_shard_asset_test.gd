...
const MODEL_ID := "bone_shard"
const GLB_PATH := "res://assets/models/materials/materials_bone_shard.glb"
const GENERATOR_PATH := "res://tools/generate_voxel_bone_shard.py"
const MANIFEST_PATH := "res://data/material_model_manifest.json"


func test_bone_shard_single_model_generator_and_asset() -> void:
	assert_bool(FileAccess.file_exists(GENERATOR_PATH)).is_true()
	var source := FileAccess.get_file_as_string(GENERATOR_PATH)
	assert_str(source).contains('MODEL_ID = "bone_shard"')
	...
	for item in parsed.get("materials", []):
		if String(item.get("id", "")) == MODEL_ID:
			found = true
			assert_str(String(item.get("generated_by", ""))).contains("generate_voxel_bone_shard.py")
			break
