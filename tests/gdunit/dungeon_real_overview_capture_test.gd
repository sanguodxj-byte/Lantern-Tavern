...
func test_capture_scene_uses_production_dungeon_without_material_gallery() -> void:
	...
	assert_bool(source.contains("_configure_capture_lighting")).is_false()
	...
	assert_bool(source.contains("dungeon_real_exploration.png")).is_true()
	assert_bool(source.contains("_find_capture_room_wall_bay")).is_true()
