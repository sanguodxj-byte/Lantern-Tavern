...
func _build_all() -> void:
	...
	for skill in AS.SKILLS:
		var tex: Texture2D = _load_pixel_icon(icon_path_for_skill_id(skill.id))
		if tex == null:
			tex = _make_action_icon()
		_cache[skill.id] = tex
	# 18 里程碑被动（按主属性映射描边色相）
	for ms in SD.ATTR_MILESTONES:
		var tex: Texture2D = _load_pixel_icon(icon_path_for_skill_id(ms.id))
		if tex == null:
			tex = _make_passive_icon(ms.attr)
		_cache[ms.id] = tex
...
func get_icon_path(skill_id: String) -> String:
	return icon_path_for_skill_id(skill_id)

static func icon_path_for_skill_id(skill_id: String) -> String:
	return "%s/skill_%s.png" % [SKILL_ICON_DIR, _stable_id_hex(skill_id)]

static func _stable_id_hex(value: String) -> String:
	return value.to_utf8_buffer().hex_encode()
