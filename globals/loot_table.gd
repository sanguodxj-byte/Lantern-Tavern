...
## 抽取一把武器。返回 {id, tier_index, tier_name, weapon_data}，空字典表示注册表未就绪。
func roll_weapon() -> Dictionary:
	...
	if tier_idx < tiers.size():
		tier_name = tiers[tier_idx].get("name", wr.get_display_name(pick_id))
	else:
		tier_name = wr.get_display_name(pick_id)
	var weapon_data = wr.get_weapon_data(pick_id)
	return {
		"id": pick_id,
		"tier_index": tier_idx,
		"tier_name": tier_name,
