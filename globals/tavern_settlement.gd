...
## 品质（一/二/三阶 tier）、装备 id、前缀词缀全部纯随机，不挂钩势力声望。
## 返回 {id, tier_index, tier_name, prefix, display_name}，空字典表示注册表未就绪。
func _pick_random_gear() -> Dictionary:
	...
	if tiers.is_empty():
		var prefix: String = _pick_random_prefix()
		var base_name: String = wr.get_display_name(pick_id)
		return {"id": pick_id, "tier_index": 0, "tier_name": base_name, "prefix": prefix, "display_name": prefix + base_name if prefix != "" else base_name}
	var tier_idx: int = randi() % tiers.size()
	var tier_name: String = tiers[tier_idx].get("name", wr.get_display_name(pick_id))
	var prefix: String = _pick_random_prefix()
	var display: String = prefix + tier_name if prefix != "" else tier_name
	return {
		"id": pick_id,
		"tier_index": tier_idx,
		"tier_name": tier_name,
