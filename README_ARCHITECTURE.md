...
> **历史参考（2026-08-03 标注）**：本文档为项目早期（OBJ 模型 / 纸娃娃装备 / 像素 UI 规约）的架构草案，目录结构与当前实现（体素 GLB / 第一人称 / 3D 实时 ARPG / GDScript）已大幅演进，**仅作历史参考**。当前实现架构以 `docs/16-技术架构与代码设计.md` 与各领域权威文档（05 战斗 / 06 装备 / 17 体素工作流 / 25 联机总体方案）为准。
...
### 4. 免费且开源的像素字库授权规约 (Open-Source Font Licensing)

为了绝对确保项目发布时的合规性，排除任何潜在的字体版权纠纷与授权成本，『灯火酒馆』在英/中/日三语中一律采用世界公认、完全免费且开源的像素字库：

1.  **英文、数字与基本符号：m5x7.ttf**
    -   **作者**: Daniel Linssen (知名独立游戏开发者/艺术家)
    -   **授权协议**: 开放共享（CC0 / Public Domain / 免费商用），无任何版权纠纷。
    -   **技术规格**: 5x7 像素网格，在微型 UI 与属性显示（如：`sweet: 5`, `Gold: 100`）中，具备无可比拟的锐利度和完美的对齐美感。已完好下载并预置于 `assets/fonts/m5x7.ttf` 供项目开箱即用。
2.  **简体中文、繁体中文、日文通用字库：方舟像素体 (Ark Pixel Font)**
    -   **授权协议**: **SIL Open Font License 1.1 (OFL-1.1 开源协议)**，对个人与商业公司完全免费，支持无限制地分发、嵌入、修改和商业化。
    -   **技术规格**: 专为游戏开发的泛中日韩（Pan-CJK）像素字库，支持 10px / 12px / 16px 像素规格。它对常用汉字覆盖极全（超过 22,000+ 字符），避免了游戏多语言环境下汉字缺失显示为“豆腐块”的通病。

通过这一开源字库架构，项目的发布版能够实现 100% 零版权成本、高美学统一度的跨端呈现！
