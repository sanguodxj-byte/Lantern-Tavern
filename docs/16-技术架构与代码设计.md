...
├── GlobalTurnManager (Autoload Mono Singleton)
└── ActiveSceneContainer (Node)
    └── [运行时加载：ExplorationScene (Node2D) 或 TavernScene (Node2D)]
```

*   **日夜场景切换控制**：
    *   **白天探索**：卸载 `TavernScene`，实例化并挂载 `ExplorationScene`。玩家在野外进行网格移动与回合制战斗。
    *   **晚上经营**：卸载 `ExplorationScene`，实例化并挂载 `TavernScene`。开启酒馆的营业状态机。
    *   **转场特写**：使用 `dave-style` 滤镜转场着色器 (Shader) 完成切屏。

### 1.2 全局单例 (Autoload Singletons)
在 Godot 中，使用 C# 单例模式访问全局节点：
...
public partial class GlobalSaveManager : Node
{
    ...
    public PlayerProgress LoadGame(string slotName);
}
```

*   `GlobalSaveManager`：控制 JSON 格式的序列化/反序列化及本地持久化。
*   `GlobalPrestigeManager`：持久化维护 5 大怪物势力的声望值、已解锁的常客名册（上限 25 名）。
*   `GlobalTurnManager`：管理白天野外探索时的 `TurnState`。玩家输入触发 `NextTurn`，同步驱动野外所有怪物的 Action Tick。
...
```
Player (CharacterBody2D)  <-- 实现 ICombatant, IDamageable 接口
├── Sprite2D (像素渲染)
├── CollisionShape2D (网格碰撞)
├── MovementComponent (Node，挂载网格移动 Tick 逻辑)
...
graph TD
    ...
    G --> H[步骤 6: 瓦片实例与 TileMapLayer 渲染]
    H --> I[生成完毕]
```
...
2.  **步骤 2：WFC (波函数崩塌) 地块拼接**
    ...
    *   每个地块模块的边缘带有连接状态（如“北通”、“南不通”）。算法在未确定格子上迭代崩塌，直至所有地块拼接完毕。
...
4.  **步骤 4：连通性校验 (Connectivity Verification)**
    ...
    *   检测所有已生成的 `Floor` 块是否可达。如果存在某些孤立的未连通区域，自动将其强行改写为 `Wall`，或者打通一条 1 格宽的连通走廊，确保玩家绝不会被困住。
