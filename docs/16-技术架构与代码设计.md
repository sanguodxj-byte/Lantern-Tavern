...
>
> **C#/Mono 表述**：全文凡出现 C#、Mono、`Newtonsoft.Json`、C# 接口驱动、`RollCriticalCheck` 等表述均属【历史参考】，当前实现全部为 GDScript；渲染目标以 §6.1 的 Forward+/Mobile 决策为准（`gl_compatibility` 仅存于历史段落）。
...
public interface ICombatant
{
    ...
    int DodgePerformance { get; } // 闪避性能：决定主动闪避 i-frame 时长/位移距离，不再以百分比削减命中率
...
### 4.1 伤害加算与命中/暴击结算算法 (CombatCalculator.cs)
配合公式压缩，在战斗中消灭任何二次乘法乘数膨胀；**命中恒为真（hitbox 接触即命中，无命中率投骰），基础伤害取骰面确定性均值**，公式完全采用 C# 的线性加算结构实现（仅暴击保留概率投骰，见 `RollCriticalCheck`）：

```csharp
public static class CombatCalculator
{
    // 命中判定：命中恒为真（hitbox 接触即命中），无命中率投骰。
    // 是否生效只由攻击 hitbox 是否覆盖到合法目标、以及防方是否处于闪避 i-frame 决定。
    public static bool ResolveHit(bool hitboxOverlap, bool defenderInIFrame)
    {
        // hitbox 未覆盖目标 → 落空；防方处于主动闪避无敌帧 → 视为不在命中区
        return hitboxOverlap && !defenderInIFrame;
    }
    ...
    public static DamageInfo CalculateMeleeDamage(ICombatant attacker, ICombatant defender)
    {
        ...
        // 1. 武器伤害骰 NdN 取确定性均值（不投骰）：dice_count * (sides+1) / 2
        int diceDamage = DiceMean(weapon); // 例：2D6 -> 2 * 7 / 2 = 7
...
### 6.1 项目渲染参数配置
> **历史参考（重要）**：下列 `gl_compatibility` / `d3d12` 表述来自早期 C#/低端兼容性方案，仅作历史参考。
> **当前决策（2026-08 定稿）**：桌面 `rendering/renderer/rendering_method = "forward_plus"`，移动端 `rendering_method.mobile = "mobile"`（见 `project.godot` 与架构审查 P0-7 平台交付闭环）；项目体素/道具材质规范禁止镜面高光。
*   **渲染驱动**：rendering_device/driver.windows = "d3d12"（历史参考，实际以项目设置/导出预设为准）
*   **渲染方式**：renderer/rendering_method = "forward_plus"（桌面）/ "mobile"（Android）；旧 `gl_compatibility` 表述为历史参考
